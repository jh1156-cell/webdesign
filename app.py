...
def load_vendor_db():
    ...
    sliced = sliced[[0, 5]]
    sliced.columns = ["거래처", "기준"]
    sliced["거래처"] = sliced["거래처"].fillna("").astype(str).str.strip()
    sliced["기준"] = sliced["기준"].fillna("").astype(str).str.strip()
...
def detect_rule(raw_rule):
    ...
    if "국내제작x1.5" in normalized:
        return "domestic_15"
    if any(alias in normalized for alias in half_14_aliases):
        return "half_14"
    return None
...
def compute_values(rule_key, x_value, final_product_price_override=None):
    ...
    if rule_key == "domestic_15":
        ...
        difference = product_price - multiplied
        sale_price = product_price * 1.1
        return {
            "A_label": "1.5배",
            "A": multiplied,
            "B_label": "상품가",
...
def render_history():
    ...
    for idx, item in enumerate(history, start=1):
        with st.expander(f"{idx}. {item['거래처']} · {item['계산식']} · {item['판매가']}", expanded=False):
            ...
            c2.markdown(f"**상품가**  \n{item['상품가']}")
            c3.markdown(f"**판매가**  \n{item['판매가']}")
            st.caption(f"기준 문구: {item['기준문구']}")

def main():
    st.title("브랜드별 단가 계산기")
    st.caption("거래처를 검색해서 선택하면, 거래처별 기준 문구에 맞춰 계산식을 자동 적용합니다.")
    ...
    st.write(f"**기준 문구:** {raw_rule if str(raw_rule).strip() else '(빈칸)'}")

    if rule_key is None:
        st.info("이 거래처는 자동 계산 대상이 아닙니다. 위 기준 문구를 확인해주세요.")
        render_history()
        st.stop()
    ...
    if rule_key == "half_14" and x_value > 0:
        ...
        if preview["editable_product_price"]:
            final_product_price_override = st.number_input(
                "상품가 직접 수정",
                min_value=float(preview["base_product_price"]),
                step=10000.0,
                value=float(preview["C"]),
                key=f"editable_c_{selected_vendor}",
            )
        else:
            st.number_input(
                "상품가 직접 수정",
                min_value=float(preview["C"]),
                step=10000.0,
                value=float(preview["C"]),
    ...
    sale_price = results["E"]
    st.markdown(
        f"""
        <div class="sale-card">
            <div class="title">최종 판매가</div>
            <div class="value">{format_currency(sale_price)}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
