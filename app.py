...
def build_margin_adjusted_result(rule_title, multiplied, base_product_price, margin_basis):
    min_gap = 30000
    base_gap = base_product_price - margin_basis
    added_gap = max(0, min_gap - base_gap)
    final_product_price = base_product_price + added_gap
    difference = final_product_price - margin_basis
    sale_price = round_to_1000(final_product_price * 1.1)
...
def compute_values(rule_key, x_value, final_product_price_override=None):
    ...
    if rule_key == "domestic_15":
        ...
        base_gap = product_price - x_value
        added_gap = max(0, min_gap - base_gap)
        final_product_price = product_price + added_gap
        return {
            "A_label": "1.5배",
            "A": multiplied,
            "B_label": "상품가",
            "B": final_product_price,
            "D_label": "차액",
            "D": final_product_price - x_value,
            "E_label": "판매가",
            "E": round_to_1000(final_product_price * 1.1),
            "base_product_label": "기본 상품가",
    ...
    if rule_key == "domestic_15_plus_40k":
        ...
        added_gap = max(0, min_gap - (product_price - x_value))
        final_product_price = product_price + added_gap
        return {
            "A_label": "1.5배",
            "A": multiplied,
