...
def ensure_margin_with_10000_round(base_product_price, margin_basis):
    target_price = max(float(base_product_price), float(margin_basis) + 30000)
    final_product_price = ceil_to_10000(target_price)
    added_gap = final_product_price - int(round(float(base_product_price)))
    difference = final_product_price - float(margin_basis)
    return final_product_price, added_gap, difference


def build_margin_adjusted_result(rule_title, multiplied, base_product_price, margin_basis):
    final_product_price, added_gap, difference = ensure_margin_with_10000_round(base_product_price, margin_basis)
    sale_price = int(round(final_product_price * 1.1))
...
def compute_values(rule_key, x_value, final_product_price_override=None):
    if rule_key == "half_14":
        ...
        suggested_product_price, suggested_added_gap, _ = ensure_margin_with_10000_round(base_product_price, purchase_price)
        ...
        sale_price = int(round(final_product_price * 1.1))
        extra_sale_from_gap = max(0, final_product_price - base_product_price) * 1.1
        return {
            "A_label": "매입가",
            "A": purchase_price,
            "B_label": "1.4배",
            ...
            "E_label": "판매가",
            "E": sale_price,
            "added_gap": max(0, final_product_price - base_product_price),
            "extra_sale_from_gap": extra_sale_from_gap,
            "editable_product_price": suggested_added_gap > 0,
            "rule_title": "반값 x1.4",
            "margin_basis": purchase_price,
        }
    ...
    if rule_key == "domestic_15":
        ...
        final_product_price, added_gap, difference = ensure_margin_with_10000_round(product_price, x_value)
        return {
            "A_label": "1.5배",
            "A": multiplied,
            "B_label": "상품가",
            ...
            "D": difference,
            "E_label": "판매가",
            "E": int(round(final_product_price * 1.1)),
            "base_product_label": "기본 상품가",
            "base_product_price": product_price,
            "added_gap": added_gap,
            "extra_sale_from_gap": added_gap * 1.1,
            "rule_title": "국내제작 x1.5",
            "margin_basis": x_value,
        }
    ...
    if rule_key == "mat_korea_ta":
        multiplied = x_value * 1.4
        base_product_price = ceil_to_10000(multiplied)
        return build_margin_adjusted_result("(MAT) 한국TA", multiplied, base_product_price, x_value)

    if rule_key == "product_code_last3_14":
        multiplied = x_value * 1.4
        base_product_price = ceil_to_10000(multiplied)
        return build_margin_adjusted_result("상품코드 뒤 3자리 x1.4 / 상품코드 단가 x1.4", multiplied, base_product_price, x_value)
...
def main():
    ...
    if rule_key in {"half_14", "domestic_15", "domestic_15_plus_40k", "project_15_plus_20k", "unit_14", "mat_korea_ta", "product_code_last3_14"}:
    ...
    with st.expander("계산식 확인", expanded=False):
        if rule_key == "half_14":
            st.markdown(
                ...
                - D = C - A = **차액**
                - 차액이 30,000원 미만이면 상품가를 30,000원 이상이 되도록 올리고 최종 상품가를 10,000원 단위로 맞춤
                - E = C × 1.1 = **판매가**
                """
            )
